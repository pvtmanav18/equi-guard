        ...
        <PageHeader 
          ...
          action={
            <div className="flex items-center gap-3">
              ...
              <button 
                title="Share Audit"
                className="tour-share-audit group p-3 rounded-2xl bg-content/[0.04] border border-content/[0.08] hover:bg-content/[0.08] transition-all hover:border-primary/30"
              >
                <Plus className="w-6 h-6 text-content/40 group-hover:text-primary transition-colors" />
              </button>
              <button 
                onMouseEnter={() => setBtnExpanded(true)}
                onMouseLeave={() => setBtnExpanded(false)}
                className={`inline-flex items-center justify-center gap-2 bg-cta text-white text-md font-semibold h-[48px] rounded-2xl transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] hover:bg-cta shadow-lg shadow-content/[0.05] overflow-hidden ${btnExpanded ? "w-[200px] px-5" : "w-[48px] px-0"}`}
              >
        ...
        <div className="tour-export-options lg:col-span-2">
          ...
          <div className="glass-card rounded-xl overflow-hidden">
            <div className="bg-content/[0.04] border-b border-content/[0.06] p-5">
              ...
              <p className="text-sm text-content/30 mt-1">{reportPreview.subtitle}</p>
            </div>
            <div className="p-5 space-y-4">{reportPreview.sections.map((section) => (<div key={section.label} className="flex items-center justify-between py-2 border-b border-content/[0.04] last:border-0"><span className="text-sm text-content/40">{section.label}</span><span className={`text-sm font-semibold ${section.color}`}>{section.value}</span></div>))}</div>
